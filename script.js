...
suggestBtn.addEventListener('click', () => {
    ...
    textContainer.textContent = randomSuggestion;

    // 创建分享链接
    const shareLink = document.createElement('span');
    shareLink.className = 'suggestion-share-link';
    shareLink.textContent = '[分享这个无用建议]';

    shareLink.addEventListener('click', async (e) => {
        e.stopPropagation();
        const fullText = `“${randomSuggestion}” —— 来自「无用建议」\n${window.location.href}`;
        try {
            if (navigator.share) {
                await navigator.share({ title: '无用建议', text: fullText });
            } else {
    ...
    // 插入两个子元素
    suggestionText.appendChild(textContainer);
    suggestionText.appendChild(shareLink);
